export default async function handler(req, res) {
    ...
    try {
        ...
        // Chiamata REST diretta a Gemini (senza dipendenze esterne che fanno crashare Vercel)
        const geminiRes = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                contents: [{
                    parts: [{ text: "Il sistema Kairós è online e operativo. Rispondi con una breve conferma in italiano." }]
                }]
            })
        });
        ...
        // Generazione TTS con Google Translate
