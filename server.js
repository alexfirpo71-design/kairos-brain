        ...
        req.on('end', async () => {
            ...
            try {
                ...
                if (activeWsClient && activeWsClient.readyState === activeWsClient.OPEN) {
                    ...
                    try {
                        await streamTextToAudio(resultText.trim(), activeWsClient, currentVolume);

                        if (activeWsClient.isSpeaking) {
                            console.log("[WS] Streaming audio dello scatto completato.");
                            if (activeWsClient.readyState === activeWsClient.OPEN) {
...
function chunkText(text) {
    const timeFormatted = formatTimeForSpeech(text);
    ...
    const sentences = sanitizedText.match(/[^.!?]+[.!?]+(\s|$)/g) || [sanitizedText];
    let chunks = [];

    for (let sentence of sentences) {
        sentence = sentence.trim();
        if (sentence.length > 200) {
            const subSentences = sentence.match(/[^,;]+[,;]+(\s|$)/g) || [sentence];
            chunks.push(...subSentences.map(s => s.trim()));
...
async function convertMp3ToPcm(mp3Buffer, volumePercent) {
...
async function streamTextToAudio(replyText, ws, volume) {
    const chunks = chunkText(replyText);

    for (const chunk of chunks) {
        if (ws.readyState !== ws.OPEN || !ws.isSpeaking) {
            break;
        }
        ...
        try {
            ...
            const pcmBuffer = await convertMp3ToPcm(mp3Buffer, volume);

            if (ws.readyState === ws.OPEN && ws.isSpeaking && pcmBuffer) {
                ...
                for (let i = 0; i < pcmBuffer.length; i += chunkSize) {
                    ...
                    ws.send(pcmBuffer.subarray(i, i + Math.min(chunkSize, pcmBuffer.length - i)), { binary: true });
                }
            }
            ...
            console.error("Errore durante il processing del blocco audio:", error);
        }
    }
}
        ...
        if (isBinary) {
            ...
            try {
                ...
                if (data.state === 'processing') {
                    ...
                    try {
                        await streamTextToAudio(replyText.trim(), ws, currentVolume);
